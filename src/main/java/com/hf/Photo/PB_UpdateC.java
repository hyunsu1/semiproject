...
@WebServlet("/PB_UpdateC")
public class PB_UpdateC extends HttpServlet {
...
//		PhotoBoardDAO.getFiles(request);
		...
		request.setAttribute("contentPage", "mg_pboard/PhotoBoardUpdate.jsp");
		request.getRequestDispatcher("jsp/index.jsp").forward(request, response);
	}

	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		AccountDAO.loginCheck(request);
//		PhotoBoardDAO.upPhotoBoard(request);
//		PhotoBoardDAO.getPhotoBoard(request);
//		PhotoBoardDAO.getFiles(request);
		PhotoBoardDAO.getPdao().upPhotoBoard(request);
		PhotoBoardDAO.getPdao().getPhotoBoard(request);
		PhotoBoardDAO.getPdao().getFiles(request);
